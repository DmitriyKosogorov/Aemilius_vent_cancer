...
MAIN_DIRECTORY='D:\Моё\учёба\Aemilius vent cancer\Blodiator-master\diagrams'
...
class BlodiatorBase(blodiatorcore.BlodiatorCore):
  ...
  def __init__(self, master, size=(WIDTH, HEIGHT), node_file=NODE_FILE_NAME,
               ...
               block_types_file=BLOCK_TYPE_FILE, std=None,grid=(0,0,1,1)):
    """
    ...
    self.__xscrollbar.grid(row=grid[0]+1, column=grid[1], sticky=tk.E+tk.W)
    self.__yscrollbar = tk.Scrollbar(master=master)
    self.__yscrollbar.grid(row=grid[0], column=grid[1]+1,  sticky=tk.N+tk.S)
    
    self.__block_type_menu = None
    
    super(BlodiatorBase, self).__init__(master=master, size=size, std=std, grid=grid)  # initialise the parent
...
#    super(BlodiatorBase, self).props()
  ...
  # keyPressed func: function handle for key pressed
  # {
  def keyPressed_im(self, key,value=None):
    """
    Internal function.
    ...
    '''
      ...
      blodiatorcore.BlodiatorCore.keyPressed_im(self, key,value)
      pass
  # } keyPressed func
...
#        self.__block_type_popup.add_radiobutton(label=block_title, command=self.setBlockType)
  ...
  def set_block_type(self, typer):
    ...
    #self.__block_type_menu.place_forget()
    self.__om_variable.set(typer)
    
    self.block_xml_file = self.__block_type_file[self.__om_variable.get()]
  # } setBlockType func

  def get_block_type(self):
    return(str(self.__om_variable.get()))
  ...
  def add_start_label(self, block):
      self.start_labels.append(block.label)
          
  def get_start_blocks_1(self):
      blocks=self.blocks
      connectors=self.connectors
      connections=[]
      result2=[]
      n=0
      blocks_unvisited=blocks
      blocks_visited=[]
      for connector in connectors:
          connections.append([connector.inPort[0][:-4], connector.outPort[0][:-4]])
      print('connectors: ', connections)
      trees=[]
      for block in blocks:
          trees.append([block.label])
      print("trees: ",trees)
      for connection in connections:
        first_block=connection[0]
        second_block=connection[1]
        move_from=-1
        move_to=-1
        for i in range(len(trees)):
            if(first_block in trees[i]):
                move_to=i
            if(second_block in trees[i]):
                move_from=i
        for block in trees[move_from]:
            trees[move_to].append(block)
        trees.pop(move_from)
      print("trees updated: ",trees)  
      start_blocks=[]
      for i in range(len(trees)):
          start_blocks.append(trees[i][1])
      print('start_blocks: ',start_blocks)
      for i in range(len(start_blocks)):
          changes=True
          while(changes==True):
              changes=False
              for connection in connections:
                  if(connection[1]==start_blocks[i]):
                      start_blocks[i]=connection[0]
                      changes=True
      print('start_blocks updated: ',start_blocks)
      return(start_blocks)
  ...
  def get_start_blocks(self):
      ...
      for block in blocks:
          ...
          if(block.function[1]=='get_command(*)'):
              start_blocks.append(block)
      return(start_blocks)
  
  def get_block_code(self,block, var_num ,block_vars,imports):
      arguments=[]
      for i in range(block.no_of_inputs):
          imp=self.find_all_connections(block.inPort[i][0],'out')
          print(imp)
          if(len(imp)>0 and imp[0][:-4] in block_vars):
            arguments.append(block_vars[imp[0][:-4]])
      arg_line=', '.join(arguments)
      function=block.function[1]
      check_lines=function.split('/')
      if(len(check_lines)>1):
          result='i'+str(var_num)+' = '+check_lines[1].replace('*', arg_line)
          if(check_lines[0] in imports.keys()):
              imports[check_lines[0]].append(check_lines[1].replace('(*)',''))
          else:
              imports[check_lines[0]]=[check_lines[1].replace('(*)','')]
          result='i'+str(var_num)+' = '+check_lines[1].replace('*', arg_line)
      else:
          result='i'+str(var_num)+' = '+block.function[1].replace('*', arg_line)
  ...
  def build_tree(self):
      ...
      connectors=self.connectors
      for connector in connectors:
          connections.append([connector.inPort[0], connector.outPort[0]])
          
      changed=True
      while(changed==True):
          changed=False
          
  def search_block_label(self,label):
      blocks=self.blocks
      for block in blocks:
  ...
  def find_all_connections(self, out, mode):
      connectors=self.connectors
      result=[]
      for connector in connectors:
          if(mode=='in' and connector.inPort[0]==out):
              result.append(connector.outPort[0])
          if(mode=='out' and connector.outPort[0]==out):
              result.append(connector.inPort[0])
      return(result)
  
  def change_com(self,alm):
      args=re.findall(r'<[A-Za-z]*>',alm)
      com_name=re.sub(r'<[A-Za-z]*>,?.?','',alm)
      return(com_name, args)
  
  def build_code(self, commands):
      start_blocks=self.get_start_blocks()
      blocks=self.blocks
      telegram_key=open("telegram_key.txt","r").readline()
      bots=['bot = telebot.TeleBot(\''+telegram_key[:-1]+'\')\n']
      with open(os.getcwd()+"\config\libs.json", "r") as read_file:
          libraries = json.load(read_file)
      block_vars={}
      for block in blocks:
          if(block.value!=None):
              block_vars[block.label]=block.value
      imports={}
      code=['@bot.message_handler(commands=["start"])\ndef start(m, res=False):\n\tbot.send_message(m.chat.id, \'Bot is activated\')\n@bot.message_handler(content_types=["text"])\ndef handle_text(message):\n']
      var_num=0
      code.append('msg = message.text')
      for i,start_block in enumerate(start_blocks):
          com_name,args=self.change_com(start_block.value)
          com_name=com_name.replace(' ','')
          print(com_name)
          print("args: ",args)
          if(i==0):
              if_line='if(msg.startswith(\''+com_name+' \')):'
          else:
              if_line='elif(msg.startswith(\''+com_name+' \')):'
          code.append(if_line)
          input_line=''
          if(len(args)==1):
              input_line='\ti'+str(var_num+1)+'= msg.replace(\''+com_name+'\',\'\')'
              code.append(input_line)
              var_num+=1
              block_vars[start_block.label]='i'+str(var_num)
          else:
              j=0
              for j in range(len(args)):
                  input_line=input_line+'i'+str(var_num+j+1)+', '
              block_vars[start_block.label]=input_line[:-2]
              input_line='\t'+input_line[:-2]+'= msg.replace(\''+com_name+'\',\'\').split(\',\')'
              code.append(input_line)
              var_num=var_num+j+1
              
          target=start_block
          in_connections=1
          while(target.get_block_name!='Output' and target.get_block_name!='Out_plot'):
              new_label=''
              #print('target1 ',target.get_block_name)
              new_label=None
              in_connections=0
              for out in target.outPort:
                  in_connections+=len(self.find_all_connections(out[0], 'in'))
              if(in_connections==0):
                  break
              for out in target.outPort:
                  connections=self.find_all_connections(out[0],'in')
                  if(len(connections)>0):
                      new_label=connections
              if(new_label!=None):             
                  var_num+=1
                  target=self.search_block_label(new_label[0][:-4]) 
                  new_line, imports=self.get_block_code(target,var_num, block_vars,imports)
                  new_line='\t'+new_line
                  code.append(new_line)
                  block_vars[target.label]='i'+str(var_num)
                  
      code.append('else:\n\t\tbot.send_message(message.chat.id, \'invalid command\')')
      print('code ',code)
      final_code='import telebot\n'
      tabs=1
      for lib in imports.keys():
          if(lib in libraries):
              if(len(libraries[lib])>1):
                  final_code =final_code+'from '+lib+' import '+libraries[lib][0]+'\n'          
                  bots.append(libraries[lib][1]+'='+libraries[lib][0]+'()\n')
              else:
                  final_code=final_code+'import '+lib+' as '+libraries[lib][0]+'\n'
          #final_code=final_code+'from '+lib+' import '+(','.join(imports[lib]))+'\n'
      for bot in bots:
          final_code=final_code+bot
      final_code=final_code+code[0]
      for i in range(1,len(code)):
          tab_line='\t'*tabs
          final_code=final_code+tab_line+code[i]+'\n'
      final_code=final_code+'bot.polling(none_stop=True, interval=0)'
      return(final_code)
  ...
  def readDiagram(self):
    ...
    f = tkFileDialog.askopenfilename(defaultextension=".dat",initialdir = MAIN_DIRECTORY)
    if f is None: # asksaveasfile return `None` if dialog closed with "cancel".
        return
    #text2save = str(text.get(1.0, END)) # starts from `1.0`, not `0.0`
    DIAG_FILE_NAME=str(f)
  ...
  def writeDiagram(self):
    ...
    output: none
    """
    f = tkFileDialog.asksaveasfile(mode='w', defaultextension=".dat",initialfile = "Diagram")
    if f is None: # asksaveasfile return `None` if dialog closed with "cancel".
        return
    #text2save = str(text.get(1.0, END)) # starts from `1.0`, not `0.0`
    DIAG_FILE_NAME=str(f.name)
  ...
  # {
  def blockSelection(self, event=None):
    """
    Internal function.
...
#        self.__block_type_popup.grab_release()
    if(event==None):
        self.__block_type_menu.place(x=0, y=0)
    else:
        self.__block_type_menu.place(x=event.x, y=event.y)    
