...
class BlodiatorCore(object):
  ...
  # {
  def __init__(self, master, size=(WIDTH, HEIGHT), std=None, grid=(0,0,1,1)):
    """
    ...
    self.__container.grid(row=grid[0], column=grid[1], rowspan=grid[2], columnspan=grid[3], sticky=tk.N+tk.S+tk.E+tk.W)
...
#    self.__container.pack()
    ...
    self.__container.bind('<Double-Button-1>', self.mouseDoubleClicked)
...
##       self.__container.config(cursor=CURSOR['node'])      
  ...
  # keyPressed_imitation func: function copy handle for key pressed
  # {
  def keyPressed_im(self, key, value=None):
    ...
    if key == KEY['print']:
      self.printItems()
    if key == KEY['block']:
       self.placeBlock(rnd.randint(300,500), rnd.randint(300,500), value)
    elif key.lower() == KEY['connector']:
       self.std.Print('Switching to connector mode', fg, bg, style, src)
       self.__OprMode = OPR_MODE['connector']
       self.__container.config(cursor=CURSOR['connector'])      
...
##       self.__container.config(cursor=CURSOR['node'])      
  ...
  def mouseMotion(self, event):
    ...
    if CONN_SNAP == True:
      for Obj in self.__Blocks:
        if (x, y) in Obj:
          ...
          if port != 0:
            change_cursor=True
            if(self.__OprMode!=OPR_MODE['connector']):
                self.__OprMode = OPR_MODE['connector']
            '''
            x, y = port.center
    ...
    if(change_cursor==True):
        self.container.config(cursor=CURSOR['connector'])
    else:
        self.container.config(cursor=CURSOR['idle'])
        
        
    if(self.__OprMode != OPR_MODE['connector'] and self.__OprMode != OPR_MODE['idle']):
        self.__OprMode=OPR_MODE['move']
  ...
  def mouseDoubleClicked(self, event):
      if(self.__OprMode != OPR_MODE['idle']):
          self.__OprMode = OPR_MODE['idle']
          self.container.config(cursor=CURSOR['idle'])
      else:
          self.__OprMode=OPR_MODE['move']
...
#    if self.__indexObj != -1 and self.__typeObj == 'Connector':
  ...
  # {
  def placeBlock(self, x, y,value=None):
    """
    Internal function.
    ...
    if(value!=None):
        #self.__tmpObj.set_block_name=value(
        #print(re.findall('<[A-Za-z]*>,*\s*', str(value)))
        new_name=re.sub('<[A-Za-z]*>,*\s*', '', str(value))
        self.__tmpObj = grfblock.GrfBlock(sheetCanvas=self.__container,\
                                      ...
                                      name=new_name, std=self.std)
        self.__tmpObj.value=value
    else:
        self.__tmpObj = grfblock.GrfBlock(sheetCanvas=self.__container,\
                                 label='Block' + str(self.__blockCounter),\
                                 xml_file=self.block_xml_file, center=(x, y),\
    ...
    print(self.__tmpObj.block_name)
